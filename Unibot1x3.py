...
def event_recommend():
    ...
    day=t.day
    month=t.month
    users_date=month+0.01*day    #convert date format from mm-dd to mm.dd like 09-22 to 9.22
...
def sub_categorized_study(tokens):
    Library_a=['struggle','difficult','difficulty','bad','help','anxiety','nervous','struggling']
    ...
    for token in tokens:                    
        ...
        for words in Library_b:
            if fire(token,words)==1:
                score_interest+=1
    if score_struggle>score_interest:
...
def sub_2_categorized_social(tokens):
    Library_type1=['artist','art',['poetry pals','painting and pottery']]
    ...
    score_board=[0,0,0,0,0]     #score_board[0] to [4] is the score index of artists, international, debate, Science&Society, and environment.
    for token in tokens:
        if fire(token,Library_type1[0])==1 or fire(token,Library_type1[1])==1:
            score_board[0]=score_board[0]+1 
        elif fire(token,Library_type2[0])==1:
            score_board[1]=score_board[1]+1 
        elif fire(token,Library_type3[0])==1:
            score_board[2]=score_board[2]+1 
        elif fire(token,Library_type4[0])==1 or fire(token,Library_type4[1])==1 :
            score_board[3]=score_board[3]+1 
        elif fire(token,Library_type5[0])==1:
            score_board[4]=score_board[4]+1 
    if score_board[0]>max(score_board[1],score_board[2],score_board[3],score_board[4]):
            print("I would like to recommend",random.choice(Library_type1[2]),'association.')
    elif score_board[1]>max(score_board[0],score_board[2],score_board[3],score_board[4]):
            print("I would like to recommend",random.choice(Library_type2[1]),'association.')
    elif score_board[2]>max(score_board[0],score_board[1],score_board[3],score_board[4]):
            print("I would like to recommend",Library_type3[1][0],'association.') 
    elif score_board[3]>max(score_board[0],score_board[1],score_board[2],score_board[4]):
            print("I would like to recommend",random.choice(Library_type4[2]),'association.') 
    elif score_board[4]>max(score_board[0],score_board[1],score_board[2],score_board[3]):
